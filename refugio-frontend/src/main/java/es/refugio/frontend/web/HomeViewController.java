...
public class HomeViewController {
    ...
    public String home(Model model) {
        // Obtener respuestas paginadas para conocer los totales reales de la base de datos
        PaginatedResponse<AnimalRecord> paginatedAnimales = 
                helper.fetchPaginated(apiUrl + "/v1/animales", 1, 1000, AnimalRecord.class);
        PaginatedResponse<VoluntarioRecord> paginatedVoluntarios = 
                helper.fetchPaginated(apiUrl + "/v1/voluntarios", 1, 1000, VoluntarioRecord.class);
        PaginatedResponse<AdopcionRecord> paginatedAdopciones = 
                helper.fetchPaginated(apiUrl + "/v1/adopciones", 1, 1000, AdopcionRecord.class);

        List<AnimalRecord> animales = paginatedAnimales.items();

        model.addAttribute(ModelAttribute.Persona_LIST.getName(),    helper.fetchList(authUrl + "/v1/usuarios", UsuarioRecord.class));
