...
*   **Validación de Estados**: Las adopciones y solicitudes solo pueden procesarse si el animal se encuentra en estado `DISPONIBLE`. Si el estado cambia a `ADOPTADO` o `RESERVADO`, las solicitudes pendientes quedan bloqueadas. Además, el ciclo de vida de la adopción gestiona automáticamente los estados: `PENDIENTE_FIRMA`, `EN_ADAPTACION`, `DEVUELTO` y `COMPLETADA/APROBADA`.
